...
class mochaLens extends abstractCodeLens {
  ...
  notificationRegister() {
    this.notification.on(this.notification.events.CONTENT_CHANGED, async editor => {
      const tests = Array.isArray(this.tests) && this.tests.find(test => test.file === editor.document.fileName);
      if (tests) {
  ...
  _createLensForTests(path) {
    const tests = Array.isArray(this.tests) && this.tests.filter(test => test.file === path);
    const lensTest = [];
    tests.forEach(test => {
