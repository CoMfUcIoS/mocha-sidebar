...
const forkFindTests = rootPath => {
  const args = {
    ...
    mochaPath: mochaPath: mochaPath(rootPath)
  };
  findingTestLogs();
  if (config.logVerbose()) {
...
const findingTestLogs = (rootPath, args) => {
  userMessage.clear();
  userMessage.send(`____________________________________________________________________________`);
  userMessage.send(`trying to searching for tests using these settings: `);
  userMessage.send(`
    ...
    requires: ${JSON.stringify(config.requires())}
`);
  userMessage.send(`if you find anything wrong please change those default settings`);
  userMessage.send(`____________________________________________________________________________`);
...
const handleProcessMessages = process => {
  return new Promise((resolve, reject) => {
    ...
    msg.on(TYPES.result, data => {
      console.log(`results came handleProcessMessages `);
      processMessage = data;
      console.log(data);
    });
...
const runTests = async (testFiles, grep, messages) => {
  ...
  //userMessage.send(`Running tests in "${rootPath}"\n`);
...
const findTests = async rootPath => {
  // Allow the user to choose a different subfolder
  //userMessage.send(`Finding tests in "${rootPath}"\n`);
