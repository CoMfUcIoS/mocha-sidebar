...
class codeCoverage {
  ...
  init(notification) {
    ...
    this.message = messages.send(messages.channelName.COVERAGE, null);
  ...
  _spawnCoverage() {
    return new Promise(async (resolve, reject) => {
      ...
      // } catch (error) {
      //   this.message.send(`cant find nyc path`);
      // }
      let path = null;
      try {
      ...
      const pathToNyc = config.nycPath();
      this._cmd = this._commandBuilder();
      const env = config.env();
      console.log(env);
      const cp = spawn(path, [pathToNyc, ...this._cmd], {
        cwd: this._workDir,
        env: { ...process.env, ...env }
      });
      ...
      cp.stderr.on("data", data => {
        // reject(data);
        this.message.send(`coverage error message`);
        this.message.send(` __________________________________________________________________________`);
        this.message.send(data);
        this.message.send(` __________________________________________________________________________`);
  ...
  _commandBuilder() {
    ...
    const mochaBin = config.mochaPath().binary;
    ...
    if (config.logVerbose()) {
      this.message.send(`coverage running the following command ${cmd}`);
    }
    return cmd;
  }
  _buildMochaCommandArgs() {
    ...
    /*
     ...
     glob: config.files().glob,
      ...
      if (config.files().ignore.length != 0) {
        config.files().ignore.forEach(req => {
          mochaOptions = mochaOptions + `--exclude ${req} `;
        });
